...
function selectCode(a){'use strict';var e=a.parentNode.parentNode.getElementsByTagName('CODE')[0];var s,r;if(window.getSelection){s=window.getSelection();if(s.setBaseAndExtent){var l=(e.innerText.length>1)?e.innerText.length-1:1;s.setBaseAndExtent(e,0,e,l);}
...
function resize(){var width=0,diff=$left.outerWidth(true)-$left.width();$right.each(function(){width+=$(this).outerWidth(true);});$left.css('max-width',Math.floor($this.width()-width-diff)+'px');}
